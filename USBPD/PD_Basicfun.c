/********************************** (C) COPYRIGHT *******************************
* File Name          : PD_Basicfun.c
* Author             : trumx
* Version            : V1.0.0
* Date               : 20240320
* Description        : PD处理函数的基础功能.
*********************************************************************************
    ...
    else                                                                /* Detect insertion */
    {
        ...
        if (USBPD->PORT_CC1 & CC_PD)
        {
            ...
            if (cmp_cc2 & bCC_CMP_22 == bCC_CMP_22)
            {
                if( ret )
                {
                    ret = 0;   /* Huawei A to C cable has two pull-up resistors */
                }
                else
                {
                    ret = 2;
 ...
 * @return  none
 */
void PD_Send_TXBUF( int len )
{
    UINT8  pd_tx_trycnt=4;
    UINT8  cnt;
